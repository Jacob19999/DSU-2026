"""
Evaluation for Pipeline D — 4-fold forward validation per eval.md contract.

Produces scored results with overall, per-fold, per-site, and per-block
WAPE breakdowns.  Saves report JSON + CSVs to output/evaluation/.
"""
...
def evaluate() -> dict:
    """Score all folds, print detailed report, save JSON + CSVs."""
    truth = _build_truth()
    ...
    # Per-fold CSV
    fold_rows = [{k: v for k, v in r.items() if k != "_merged"} for r in fold_results]
