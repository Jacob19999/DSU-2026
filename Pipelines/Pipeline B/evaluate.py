"""
Evaluation for Pipeline B — 4-fold forward validation per eval.md contract.
...
def _score_fold(truth: pd.DataFrame, fold: dict) -> dict | None:
    ...
    # Per-bucket breakdown (assign bucket from days_ahead = date - train_end)
    merged["_date"] = pd.to_datetime(merged["Date"])
    ...
    bucket_wapes = {}
    for bid in [1, 2, 3]:
        bsub = merged[merged["_bucket"] == bid]
        if len(bsub) > 0:
            bucket_wapes[bid] = {
                "total_wape":    wape(bsub["ED Enc_true"].values, bsub["ED Enc_pred"].values),
                "admitted_wape": wape(bsub["ED Enc Admitted_true"].values,
                                      bsub["ED Enc Admitted_pred"].values),
                "n_rows": len(bsub),
            }
...
def evaluate() -> dict:
    ...
    sep = "=" * 68
    print(f"\n{sep}")
    print(" PIPELINE B: DIRECT MULTI-STEP GBDT — EVALUATION REPORT")
    print(sep)
    ...
    print(f"\n PER-BUCKET (mean across folds):")
    for bid in [1, 2, 3]:
        b_aw = [r["bucket_wapes"].get(bid, {}).get("admitted_wape", float("nan"))
                for r in fold_results if bid in r.get("bucket_wapes", {})]
        b_tw = [r["bucket_wapes"].get(bid, {}).get("total_wape", float("nan"))
                for r in fold_results if bid in r.get("bucket_wapes", {})]
        if b_aw:
            b_cfg = cfg.BUCKETS[bid]
            print(f"   Bucket {bid} (h={b_cfg['h_min']}-{b_cfg['h_max']}): "
                  f"admitted_wape={np.nanmean(b_aw):.4f}  "
                  f"total_wape={np.nanmean(b_tw):.4f}")
    ...
    for site in cfg.SITES:
        sub = all_merged[all_merged["Site"] == site]
        if len(sub) > 0:
            site_wapes[site] = wape(sub["ED Enc Admitted_true"].values,
    ...
    if site_wapes:
        ...
        if worst_site > 2 * best_site:
            print(f"\n  WARNING: worst site WAPE ({worst_site:.4f}) > 2x best ({best_site:.4f})")

    fold_var = np.std([r["admitted_wape"] for r in fold_results]) / mean_aw if mean_aw > 0 else 0
    if fold_var > 0.3:
    ...
    fold_rows = []
    for r in fold_results:
        row = {k: v for k, v in r.items() if k not in ("_merged", "bucket_wapes")}
